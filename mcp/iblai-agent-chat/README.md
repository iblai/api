...
The iblai-api-agent-chat server provides a bridge between MCP-compatible AI assistants (like Claude) and ibl.ai's mentor system. This allows users to interact with specialized AI mentors configured for specific domains, courses, or purposes through their preferred AI assistant interface.
...
```bash
claude mcp add iblai-api-agent-chat --transport http https://asgi.data.iblai.app/mcp/agent-chat/ --header "Authorization: Api-Token YOUR_API_TOKEN" --header "X-Mentor-Unique-Id: YOUR_MENTOR_UNIQUE_ID"
```
...
3. **Via iblai-api-search**: Use the mentor search tools to discover mentors and their IDs
...
| Server | Description |
|--------|-------------|
| [iblai-api-analytics](../iblai-api-analytics) | Monitor mentor usage and costs |
| [iblai-api-search](../iblai-api-search) | Discover available mentors |
| [iblai-api-agent-create](../iblai-api-agent-create) | Create and manage AI mentors |
